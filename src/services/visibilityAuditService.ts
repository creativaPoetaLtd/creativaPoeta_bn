...
export class VisibilityAuditError extends Error {
...
type FetchResult = { finalUrl: string; html: string; status: number; contentType: string; redirects: number; responseTimeMs: number };
type AuditCheck = { id: string; label: string; passed: boolean; weight: number; detail: string; priority: string };
...
const parsePublicUrl = (input: string) => {
  const value = input.trim();
  const normalized = /^https?:\/\//i.test(value) ? value : `https://${value}`;
  let url: URL;
  try { url = new URL(normalized); } catch { throw new VisibilityAuditError("L'adresse du site n'est pas valide."); }
  if (!["http:", "https:"].includes(url.protocol)) throw new VisibilityAuditError("Seuls les sites HTTP et HTTPS peuvent etre analyses.");
  if (url.username || url.password) throw new VisibilityAuditError("Les adresses contenant des identifiants sont refusees.");
  if (!url.hostname || url.hostname === "localhost" || url.hostname.endsWith(".local")) throw new VisibilityAuditError("Cette adresse ne peut pas etre analysee.");
  url.hash = "";
  return url;
};
...
const resolvePublicAddress = async (hostname: string) => {
  ...
  const addresses = directVersion ? [{ address: hostname, family: directVersion }] : await lookup(hostname, { all: true, verbatim: true });
  if (!addresses.length || addresses.some((item) => isBlockedIp(item.address))) throw new VisibilityAuditError("Le site pointe vers une adresse reseau non autorisee.");
  return addresses[0];
};

const requestHtml = async (url: URL, redirects = 0): Promise<FetchResult> => {
  const startedAt = Date.now();
  let address: Awaited<ReturnType<typeof resolvePublicAddress>>;

  try {
    address = await resolvePublicAddress(url.hostname);
  } catch (error) {
    if (error instanceof VisibilityAuditError) throw error;
    throw new VisibilityAuditError(
      "Impossible de resoudre l'adresse de ce site.",
      422
    );
  }

  const transport = url.protocol === "https:" ? https : http;
  const pinnedLookup: LookupFunction = (_hostname, _options, callback) => callback(null, address.address, address.family as 4 | 6);
  ...
  return new Promise((resolve, reject) => {
    const request = transport.request(url, {
      ...
      if (status >= 300 && status < 400 && location) {
        response.resume();
        if (redirects >= MAX_REDIRECTS) return reject(new VisibilityAuditError("Le site effectue trop de redirections.", 422));
        try { resolve(await requestHtml(parsePublicUrl(new URL(location, url).toString()), redirects + 1)); } catch (error) { reject(error); }
        return;
      }
      if (status < 200 || status >= 400) { response.resume(); return reject(new VisibilityAuditError(`Le site a repondu avec le statut HTTP ${status}.`, 422)); }
      const contentType = String(response.headers["content-type"] || "");
      if (!contentType.toLowerCase().includes("html")) { response.resume(); return reject(new VisibilityAuditError("L'adresse ne renvoie pas une page HTML.", 422)); }
      if (Number(response.headers["content-length"] || 0) > MAX_HTML_BYTES) { response.destroy(); return reject(new VisibilityAuditError("La page est trop volumineuse.", 422)); }
      const chunks: Buffer[] = [];
      let bytes = 0;
      response.on("data", (chunk: Buffer) => { bytes += chunk.length; if (bytes > MAX_HTML_BYTES) response.destroy(new VisibilityAuditError("La page est trop volumineuse.", 422)); else chunks.push(chunk); });
...
export const runTechnicalVisibilityAudit = async (rawUrl: string) => {
  ...
  $("script[type='application/ld+json']").each((_i, el) => {
    try {
      ...
      (Array.isArray(parsed) ? parsed : [parsed]).forEach((item) => {
        ...
        types.filter(Boolean).forEach((type: string) => structuredDataTypes.add(type));
      });
