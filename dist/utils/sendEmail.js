...
const isConfigured = (value) => Boolean(value && value.trim());
...
const getExtraMailboxKeys = () => {
    ...
    const discoveredKeys = Object.keys(process.env)
        .map((key) => { var _a; return (_a = key.match(/^(SMTP|IMAP)_(.+)_(USER|ADDRESS|EMAIL)$/)) === null || _a === void 0 ? void 0 : _a[2]; })
        .filter((key) => Boolean(key))
        .map(normalizeEnvKey);
    return Array.from(new Set([...declaredKeys, ...discoveredKeys]));
};
const getMailboxSmtpConfigs = () => {
    const configs = [];
    ...
    getExtraMailboxKeys().forEach((key) => {
        const user = process.env[`SMTP_${key}_USER`] ||
...
const resolveSmtpConfig = (fromEmail) => {
    const configs = getMailboxSmtpConfigs();
    const normalizedFrom = String(fromEmail || "").toLowerCase().trim();
    if (normalizedFrom) {
        const matchingConfig = configs.find((config) => [config.user, config.fromEmail]
            .filter(Boolean)
...
const sendEmail = async (to, subject, htmlContent, attachmentsOrOptions) => {
    try {
        ...
        const html = (0, emailTemplate_1.renderBrandedEmail)({
